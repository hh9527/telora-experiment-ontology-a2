...
- 文件内容是比旧完成报告更新的状态来源。每次 A2 处理反馈后，记录当时完整的
  `ent-1/FEEDBACK.md` 为“A2 已处理版本”；每次 A3 完成复验后，记录当时完整的
  `ontology/DSL-TUTORIAL.md`、`ontology/AI3-CONTRACT.md` 和
  `ontology/NOTES.md` 为“A3 已复验版本”。记录仅用于当前会话中的内容比较，
  不写入 workspace。
...
- **依据什么**：会话树、已记录的 session ID、重新读取的公开交付完整内容、各
  角色最近一次已处理/已复验的内容版本，以及子代理完成报告。不能只依据上一次
  A3 返回摘要中的“完成”或“无新问题”。
- **做什么事情**：开始新的执行段并将轮次清零；在匹配 C2-C8 之前，必须重新读取
  `ent-1/FEEDBACK.md`、`ontology/DSL-TUTORIAL.md`、
  `ontology/AI3-CONTRACT.md` 和 `ontology/NOTES.md`，逐字比较当前内容与对应的
  最近已处理版本：
  1. 若 `ent-1/FEEDBACK.md` 已变化，把它视为尚未由 A2 处理的新反馈，匹配 C7；
  2. 否则，若任一 ontology 公开交付已变化，把它视为尚未由 A3 复验的上游更新，
     匹配 C8；
  3. 只有确认这些文件均未变化后，才按旧完成报告判断任务是否仍满足挂起条件。
  若恢复时最近一次 A3 返回已经在上一执行段结算，不重复计数；文件变化触发的
  新 A2/A3 返回按新执行段正常结算。
- **做到什么算好**：复用已经存在的 A2/A3 session，且再次满足挂起条件前最多
  完成 3 轮；任何发生在挂起期间的公开文件变化都已经被对应角色处理或复验，
  不会因旧完成报告而被遗漏。
