...
Plan := Model(QueryIntent)  # 无法满足契约时 fail!，诊断由 Host 观察
...
关系目录表达领域事实，不得为了让某个基点的遍历形成 DAG 而删除有效反向关系。
...
这些是可观察要求，并非要求使用某一种搜索算法。实现必须对有向环稳定：已经访问
的节点不能重新进入待展开前沿，环本身不构成截断。
...
当任一有界遍历到达八条边后仍有尚未访问的后继时，使用 `fail!` 产生截断诊断。
当且仅当配置的界限可能隐藏更远的可达性时才算截断；只通向已访问节点的环边不算
未展开前沿。截断后不得发布 Plan。fuel 耗尽是运行时失败，不是截断诊断。
...
Model 必须把 QueryIntent 视为一个原子请求：任一必要能力或路径失败时，不发布
...
包含模型 revision、能力目录、基础数据源、度量与维度表达式、关系目录与物理映射，
以及公共 API 要求的有类型 selector 或相等性能力。

模型是一个查询编译器，不只是领域目录。给定 `QueryIntent`，模型必须执行本文的
能力、路径、诊断和发布协议：成功时返回规范 Plan，失败时通过 `fail!` 产生 Host
诊断且不产生值。企业不得通过一个任意的最终 builder 回调手工构造 Plan，也不得在
模型外重复共享编译规则。
...
make_plan_creator: Model -> Fn(QueryIntent) -> Plan
make_query_creator: Model -> Fn(QueryIntent) -> SqlQuery
```
...
公共模型 API 只承诺成功值 `Plan` 或 `SqlQuery`。当能力不完整、路径只能扇出、
路径缺失、遍历截断或规范 Plan 无法完整组装时，eDSL 使用 `fail!`；普通 Telora
调用者不接收 `Rejection`、`BlameError`、`Outcome` 或诊断数组。

适用时，诊断保留下列三个来源：

1. 意图：请求的标识；
2. 模型事实：涉及的能力或关系；
3. 共享规则：判定失败的 eDSL 检查。

这些来源通过 `fail!` 的有序 subjects 及其来源元数据交给 Host，而不是复制成领域
错误协议。message 应稳定描述规则，subjects 保留请求、能力或关系等结构化事实。

彼此独立的诊断先于最终发布决策运行。过早的 gate 不得掩盖无关的失败：例如，一个
请求缺少 capability 时，仍须分类其他能够直接从 QueryIntent 确定的目标路径。
普通源码只声明依赖和失败点；求值器与 Host 决定如何尽力继续独立工作。
...
否则，最终结果为失败产生的 `Never`，并且不发布部分计划、SQL 文本或 bindings。
